...
mysqli_query($link,$carga) or die (mysql_error());
	   ...
	   $gruposinstituto = mysqli_query($link,"SELECT * FROM matriculas ");

while($row = mysqli_fetch_array($gruposinstituto)) {
$grupoclase=$row['grupo'];
$cadenaComprueba="SELECT * FROM gruposinf WHERE nombre=".$grupoclase;
...
if (mysqli_num_rows($compruebagrupo)==0){
//echo "<p>Grupo nuevo. En matriculas ".$grupoclase." en gruposinf: ".$compruebagrupo['nombre']."</php> ".'\n';


mysqli_query($link,"insert into gruposinf (nombre) values ('$grupoclase')");}
	}
